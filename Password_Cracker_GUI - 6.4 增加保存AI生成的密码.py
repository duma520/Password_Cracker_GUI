...
class PasswordCrackerGUI(QMainWindow):
    ...
    def show_dict_list_context_menu(self, pos):
        ...
        # 按文件名排序
        sort_by_name_action = QAction("按文件名", self)
        sort_by_name_action.triggered.connect(lambda: self.sort_dict_items(by='name'))
        
        # 按文件大小排序
        sort_by_size_action = QAction("按文件大小", self)
        sort_by_size_action.triggered.connect(lambda: self.sort_dict_items(by='size'))
    ...
    def sort_dict_items(self, by='name'):
        """按指定方式排序字典列表"""
        items = []
        for i in range(self.dict_list.count()):
            item = self.dict_list.item(i)
        ...
        if by == 'name':
            ...
            items.sort(key=lambda x: (-x['size'], x['name'].lower()))  # 降序排列
        ...
        for item_data in items:
            ...
            if os.path.isfile(path):
                ...
                new_item.setText(f"{os.path.basename(path)} ({size_str})")
