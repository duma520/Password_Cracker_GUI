...
class PasswordCrackerGUI(QMainWindow):
    ...
    def init_ai_learning_ui(self):
        ...
        self.ai_clear_btn.clicked.connect(self.ai_dict_list.clear)
    ...
    def start_ai_learning(self):
        """开始AI学习并生成密码"""
        selected_items = [self.ai_dict_list.item(i).data(Qt.UserRole) 
                         for i in range(self.ai_dict_list.count()) 
                         if self.ai_dict_list.item(i).isSelected()]
    ...
    def get_size_color(self, size):
        ...
        # 对数比例处理（核心）
        log_size = math.log10(max(1, size))          # 避免log(0)
        log_max = math.log10(1024 * 1024 * 1024 * 100)  # 100GB基准
        raw_ratio = min(log_size / log_max, 1.0)     # 基础比例[0,1]
        
        # 黄金分割非线性映射（关键改进点）
        golden_ratio = phi**3 + raw_ratio * (1 - phi**3)  # 在[φ³,1]区间映射
        
        # HSL颜色空间（青→绿→红）
        hue = 180 - (180 * golden_ratio)  # 色相：青蓝(180°) → 红(0°)
        saturation = 0.9                  # 饱和度保持鲜艳
        lightness = 0.5 - (0.2 * golden_ratio)  # 亮度微调
    ...
    def init_ui(self):
        ...
        # AI设置选项卡
        ai_tab = QWidget()
        ai_layout = QVBoxLayout()  # 确保AI选项卡有布局
        ai_tab.setLayout(ai_layout)  # 设置布局
        ...
        ai_layout.addStretch()  # 添加拉伸以保持控件在顶部
        ...
        deselect_all_archive_btn = QPushButton("全不选")
        deselect_all_archive_btn.clicked.connect(lambda: self.toggle_all_archive_items(False))
        ...
        archive_button_layout.addWidget(deselect_all_archive_btn)
        ...
        deselect_all_dict_btn = QPushButton("全不选")
        deselect_all_dict_btn.clicked.connect(lambda: self.toggle_all_dict_items(False))
        ...
        dict_button_layout.addWidget(deselect_all_dict_btn)
