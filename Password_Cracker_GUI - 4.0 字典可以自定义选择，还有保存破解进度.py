    ...
    def process_dictionary(self, dict_path, dict_index):
        try:
            ...
            for i, line in enumerate(lines):
                ...
                self.status_message.emit(f"正在尝试: {password} (字典 {dict_index+1}/{len(self.dictionary_paths)}, 行 {i+1})")
                progress = int((self.tried_passwords / self.total_passwords) * 100)
                self.progress_updated.emit(progress, self.tried_passwords, dict_index)
...
class PasswordCrackerGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        dict_layout.addWidget(self.progress_detail_label)
        ...
        self.start_btn.clicked.connect(lambda: self.start_cracking(False))
    ...
    def save_resume_info(self):
        if self.cracker_thread and hasattr(self.cracker_thread, 'resume_info'):
            resume_data = {
                "archive_path": self.archive_path_edit.text(),
                "sevenz_path": self.sevenz_path_edit.text(),
                "recursive": self.recursive_check.isChecked(),
                "resume_info": self.cracker_thread.resume_info,
                "dict_paths": [self.dict_list.item(i).data(Qt.UserRole) 
                              for i in range(self.dict_list.count()) 
                              if self.dict_list.item(i).checkState() == Qt.Checked],
                "total_passwords": getattr(self.cracker_thread, 'total_passwords', 0),
                "tried_passwords": getattr(self.cracker_thread, 'tried_passwords', 0)
            }
    ...
    def start_cracking(self, selected_only=False):
        ...
        self.cracker_thread.current_file_changed.connect(self.update_current_dict)
    ...
    def resume_cracking(self):
        ...
        # 设置基本参数
        self.archive_path_edit.setText(resume_data.get("archive_path", ""))
        ...
        # 检查文件是否存在
        if not os.path.exists(resume_data.get("archive_path", "")):
            QMessageBox.warning(self, "警告", "压缩文件不存在，无法恢复")
            return

        if not os.path.exists(resume_data.get("sevenz_path", "7z.exe")):
            QMessageBox.warning(self, "警告", "7z.exe不存在，无法恢复")
            return

        self.status_display.clear()
        self.status_display.append(f"恢复破解: {resume_data.get('archive_path', '')}")
        self.status_display.append(f"上次进度: 已尝试 {resume_data.get('tried_passwords', 0)}/{resume_data.get('total_passwords', 0)} 个密码")

        self.cracker_thread = ArchiveCracker(
            resume_data["archive_path"],
            resume_data["dict_paths"],
            resume_data["recursive"],
            resume_data["sevenz_path"],
    ...
    def update_progress(self, value, current, file_index):
        self.progress_bar.setValue(value)
        total = getattr(self.cracker_thread, 'total_passwords', 1)
        self.progress_detail_label.setText(
            f"进度: {current}/{total} ({value}%) | 当前字典: {file_index+1}/{len(self.cracker_thread.dictionary_paths)}")

    def update_current_dict(self, dict_name):
        self.current_dict_label.setText(f"当前字典: {dict_name}")
    ...
    def cracking_finished(self, success):
        ...
        # 删除恢复文件
        if success and os.path.exists(self.resume_file):
            try:
                os.remove(self.resume_file)
            except:
                pass
