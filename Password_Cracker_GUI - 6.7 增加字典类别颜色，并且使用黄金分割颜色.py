...
class PasswordCrackerGUI(QMainWindow):
    ...
    def get_size_color(self, size):
        """根据文件大小生成渐变色（黄金分割比例优化）"""
        if size == 0:  # 目录
            return QColor(150, 150, 150)  # 固定灰色
        
        # 黄金分割比例 φ = (1 + √5)/2 ≈ 0.618
        phi = 0.618
        
        # 动态计算最大参考值（基于实际文件大小的对数比例）
        max_size = 1024 * 1024 * 10  # 10MB基础参考
        log_size = math.log10(max(1, size))  # 避免log(0)
        log_max = math.log10(max_size)
        
        # 使用黄金分割创建非线性比例
        ratio = min(log_size / log_max, 1.0)
        golden_ratio = (math.pow(phi, 3) + ratio * (1 - math.pow(phi, 3)))  # 非线性映射
        
        # HSL颜色空间（从青蓝→绿→黄→红）
        hue = 180 - (180 * golden_ratio)  # 180°(青蓝)到0°(红)
        saturation = 0.9  # 高饱和度
        lightness = 0.5 - (0.2 * golden_ratio)  # 轻微明暗变化
