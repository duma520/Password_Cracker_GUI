...
class PasswordCrackerGUI(QMainWindow):
    ...
    def move_ai_dict_item_up(self):
        current_row = self.ai_dict_list.currentRow()
        if current_row > 0:
            item = self.ai_dict_list.takeItem(current_row)
            self.ai_dict_list.insertItem(current_row - 1, item)
            self.ai_dict_list.setCurrentRow(current_row - 1)

    def move_ai_dict_item_down(self):
        current_row = self.ai_dict_list.currentRow()
        if current_row < self.ai_dict_list.count() - 1:
            item = self.ai_dict_list.takeItem(current_row)
            self.ai_dict_list.insertItem(current_row + 1, item)
            self.ai_dict_list.setCurrentRow(current_row + 1)

    def toggle_ai_dict_item_selection(self):
    ...
    def sort_ai_dict_items(self, by='name_asc'):
        """按指定方式排序AI字典列表"""
        items = []
        for i in range(self.ai_dict_list.count()):
            ...
            file_info = {
                ...
                'size': 0,
                'mtime': 0,
                'type': os.path.splitext(path)[1].lower() if os.path.isfile(path) else 'dir'
            }
    ...
    def show_ai_dict_context_menu(self, pos):
        ...
        move_up_action.triggered.connect(self.move_ai_dict_item_up)
        menu.addAction(move_up_action)
        
        move_down_action = QAction("下移", self)
        move_down_action.triggered.connect(self.move_ai_dict_item_down)
        ...
        sort_by_name_action.triggered.connect(lambda: self.sort_ai_dict_items('name_asc'))
        
        sort_by_name_desc_action = QAction("按文件名(Z-A)", self)
        sort_by_name_desc_action.triggered.connect(lambda: self.sort_ai_dict_items('name_desc'))
        
        sort_by_size_action = QAction("按文件大小(小→大)", self)
        sort_by_size_action.triggered.connect(lambda: self.sort_ai_dict_items('size_asc'))
        
        sort_by_size_desc_action = QAction("按文件大小(大→小)", self)
        sort_by_size_desc_action.triggered.connect(lambda: self.sort_ai_dict_items('size_desc'))
        
        sort_by_mtime_action = QAction("按修改时间(旧→新)", self)
        sort_by_mtime_action.triggered.connect(lambda: self.sort_ai_dict_items('mtime_asc'))
        
        sort_by_mtime_desc_action = QAction("按修改时间(新→旧)", self)
        sort_by_mtime_desc_action.triggered.connect(lambda: self.sort_ai_dict_items('mtime_desc'))
        
        sort_by_type_action = QAction("按文件类型", self)
        sort_by_type_action.triggered.connect(lambda: self.sort_ai_dict_items('type'))
        ...
        toggle_action.triggered.connect(self.toggle_ai_dict_item_selection)
    ...
    def init_ai_learning_ui(self):
        ...
        self.ai_select_all_btn = QPushButton("全选")
        self.ai_select_all_btn.clicked.connect(lambda: self.toggle_all_ai_dict_items(True))
        self.ai_select_none_btn = QPushButton("全不选")
        self.ai_select_none_btn.clicked.connect(lambda: self.toggle_all_ai_dict_items(False))
        ...
        ai_dict_btn_layout.addWidget(self.ai_select_all_btn)
        ai_dict_btn_layout.addWidget(self.ai_select_none_btn)
    ...
    def init_ui(self):
        ...
        select_all_archive_btn = QPushButton("全选")
        select_all_archive_btn.clicked.connect(lambda: self.toggle_all_archive_items(True))
        select_none_archive_btn = QPushButton("全不选")
        select_none_archive_btn.clicked.connect(lambda: self.toggle_all_archive_items(False))
        ...
        archive_button_layout.addWidget(select_all_archive_btn)
        archive_button_layout.addWidget(select_none_archive_btn)
        ...
        select_all_dict_btn = QPushButton("全选")
        select_all_dict_btn.clicked.connect(lambda: self.toggle_all_dict_items(True))
        select_none_dict_btn = QPushButton("全不选")
        select_none_dict_btn.clicked.connect(lambda: self.toggle_all_dict_items(False))
        ...
        dict_button_layout.addWidget(select_all_dict_btn)
        dict_button_layout.addWidget(select_none_dict_btn)
    ...
    def toggle_all_ai_dict_items(self, checked):
        """切换所有AI字典的选择状态"""
        for i in range(self.ai_dict_list.count()):
            item = self.ai_dict_list.item(i)
            item.setCheckState(Qt.Checked if checked else Qt.Unchecked)
    ...
    def load_settings(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                for item_data in config.get("dict_items", []):
                    ...
                    if os.path.isfile(path):
                        try:
                            ...
                            item.setText(f"{os.path.basename(path)} ({size_str}, {mtime_str})")
                            item.setIcon(QIcon.fromTheme("text-x-generic"))
                        except:
                            item.setText(f"{os.path.basename(path)} (大小未知)")
                    elif os.path.isdir(path):
                ...
                # 加载AI字典列表
                self.ai_dict_list.clear()
                for item_data in config.get("ai_dict_items", []):
                    if os.path.exists(item_data["path"]):
                        ...
                        # 设置AI字典颜色
                        try:
                            size = os.path.getsize(item_data["path"])
                            color = self.get_size_color(size)
                            item.setForeground(color)
                        except:
                            pass
